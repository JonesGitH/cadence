...
def student_import_template():
    ...
    COLS = [
        ...
        ('services',        'Services',             40, 'Comma-separated keys: written_expression, grammar, spelling, reading, reading_comprehension, executive_functioning, middle_school_math, elementary_math, algebra, geometry, algebra2, precal, trig, high_school_math, other'),
    ...
    grades_str = ','.join(['K','1st','2nd','3rd','4th','5th','6th','7th',
                            '8th','9th','10th','11th','12th','College','Other'])
    dv_grade = DataValidation(type='list', formula1=f'"{grades_str}"',
    ...
    return send_file(buf, mimetype='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
                     as_attachment=True, download_name='cadence_students_template.xlsx')
...
def import_students():
    ...
    # ── Detect header row (skip note rows at top) ─────────────────────────────
    COL_KEYS = [
        'name', 'initials', 'email', 'phone', 'school', 'grade', 'birthday',
        'diagnosis', 'services', 'services_other', 'start_date', 'end_date',
        'test_date', 'parent1_name', 'parent2_name', 'parent_address',
    ...
    for row_idx, row in enumerate(rows):
        ...
        # Check if this row looks like our header
        if 'name' in row_lower and 'initials' in row_lower:
            ...
            # (identified by the name cell being longer than a real name or
            #  the initials cell being more than 5 characters)
    ...
    def _date_cell(row, key):
        """Return YYYY-MM-DD string or empty. Handles both date objects and strings."""
        v = row[header_map[key]] if header_map.get(key) is not None and header_map[key] < len(row) else None
        if v is None: return ''
        ...
        for fmt in ('%Y-%m-%d', '%m/%d/%Y', '%m/%d/%y', '%d/%m/%Y'):
            try:
                ...
                pass
        return s  # pass through as-is; DB accepts any string

    conn = get_db()
    existing = {r['name'].strip().lower() for r in conn.execute('SELECT name FROM clients').fetchall()}
    ...
    for row_idx, row in enumerate(rows[data_start_row:], start=data_start_row + 1):
        ...
        # Parse services: comma-separated keys
        ...
        services_json = json.dumps(svc_list) if svc_list else json.dumps([])
        ...
        try:
            ...
            ''', (name, initials,
                  ...
                  services_json, _cell(row, 'services_other'),
    ...
    for em in error_msgs[:5]:  # cap at 5 inline error messages
        flash(em, 'error')
